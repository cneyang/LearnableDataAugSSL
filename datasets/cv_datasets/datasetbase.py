...
class NonAugmentedDataset(Dataset):
    """
    NonAugmentedDataset returns a pair of image and labels (targets).
    If targets are not given, NonAugmentedDataset returns None as the label.
    This class return non-augmented images.
    """
    ...
    def __init__(self,
        ...
        Args
            ...
            onehot: If True, label is converted into onehot vector.
        """
        super(NonAugmentedDataset, self).__init__()
        self.alg = alg
        self.data = data
        self.targets = targets
    ...
    def __getitem__(self, idx):
        ...
        if not self.is_ulb:
            return {'idx_lb': idx, 'x_lb': transforms.ToTensor()(img), 'y_lb': target} 
        else:
            return {'idx_ulb': idx, 'x_ulb': transforms.ToTensor()(img)} 
