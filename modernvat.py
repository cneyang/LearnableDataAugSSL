...
class ModernVAT(AlgorithmBase):
    ...
    def __init__(self, args, net_builder, tb_log=None, logger=None):
        ...
        self.init(unsup_warm_up=args.unsup_warm_up, vat_eps=args.vat_eps, T=args.T, p_cutoff=args.p_cutoff, vat_embed=args.vat_embed, hard_label=args.hard_label)
        self.lambda_ent = args.ent_loss_ratio

    def init(self, T, p_cutoff, unsup_warm_up=0.4, vat_eps=6, vat_embed=False, hard_label=True):
    ...
    def train_step(self, x_lb, y_lb, x_ulb_w):
        ...
        with self.amp_cm():
            ...
            if self.vat_embed:
                ...
                unsup_loss = self.vat_loss(self.model, x_ulb_w, logits_x_ulb_w, eps=self.vat_eps)
                self.bn_controller.unfreeze_bn(self.model)

            loss_entmin = self.entropy_loss(pseudo_label)
