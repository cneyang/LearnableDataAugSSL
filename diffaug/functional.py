...
class _STE(Function):
    ...
    @staticmethod
    def forward(
        ctx, input_forward: torch.Tensor, input_backward: torch.Tensor
    ) -> torch.Tensor:
...
def tensor_function(func):
    ...
    def inner(*args):
        ...
        if not torch.is_tensor(img):
            raise RuntimeError(
                f"img is expected to be torch.Tensor, but got {type(img)} instead"
            )
...
def _blend_image(img1: torch.Tensor, img2: torch.Tensor, alpha: torch.Tensor):
    # blend two images
...
def _blur(img: torch.Tensor, kernel: torch.Tensor) -> torch.Tensor:
    assert kernel.ndim == 2
    c = img.size(1)
    return F.conv2d(
        F.pad(img, (1, 1, 1, 1), "reflect"),
...
@tensor_function
def hflip(img: torch.Tensor, _=None) -> torch.Tensor:
    return img.flip(dims=[3])


@tensor_function
def vflip(img: torch.Tensor, _=None) -> torch.Tensor:
...
@tensor_function
def invert(img: torch.Tensor, _=None) -> torch.Tensor:
    return 1 - img
...
@tensor_function
def posterize(img: torch.Tensor, mag: torch.Tensor) -> torch.Tensor:
    # mag: 0 to 1
    mag = mag.view(-1, 1, 1, 1)
    with torch.no_grad():
        shift = ((1 - mag) * 8).long()
...
@tensor_function
def gray(img: torch.Tensor, _=None) -> torch.Tensor:
    return _gray(img).repeat(1, 3, 1, 1)


@tensor_function
def contrast(img: torch.Tensor, mag: torch.Tensor) -> torch.Tensor:
    mean = (
...
@tensor_function
def auto_contrast(img: torch.Tensor, _=None) -> torch.Tensor:
    with torch.no_grad():
        # BxCxHxW -> BCxHW
        # temporal fix
        ...
        output = (torch.arange(256, device=img.device, dtype=img.dtype) - min) * (
            255 / (max - min + 0.1)
        )
...
@tensor_function
def hue(img: torch.Tensor, mag: torch.Tensor) -> torch.Tensor:
...
@tensor_function
def sample_pairing(img: torch.Tensor, mag: torch.Tensor) -> torch.Tensor:
...
@tensor_function
def equalize(img: torch.Tensor, _=None) -> torch.Tensor:
    ...
    with torch.no_grad():
        ...
        shifted = reshaped + 256 * torch.arange(
            0, size, device=reshaped.device, dtype=reshaped.dtype
        ).view(-1, 1, 1)
        ...
        cdf = (
            torch.cat([cdf.new_zeros((cdf.size(0), 1)), cdf], dim=1)[:, :256]
            + (step / 2).floor_()
        )
        # to avoid zero-div, add 0.1
        output = (cdf / (step + 0.1)).floor_().view(-1)[shifted.long()].reshape_as(
            img
        ) / 255
    return ste(output, img)
...
@tensor_function
def sharpness(
    img: torch.Tensor, mag: torch.Tensor, kernel: Optional[torch.Tensor] = None
) -> torch.Tensor:
    if kernel is None:
